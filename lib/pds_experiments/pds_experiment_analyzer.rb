  ...
  class PDSExperimentAnalyzer
    ...
    def analyze
...

      {
        ...
        date_of_birth_discrepancies: counters['date_of_birth_discrepancies'] || 0,
        summary: generate_summary(counters, total, successful, coverage_percentage)
      }
    end

    def self.analyze_all_experiments
      PDSExperiments::PDSExperimentRunner::EXPERIMENTS.keys.map do |experiment_name|
    ...
    def self.compare_experiments
      ...
      puts "%-30s %10s %10s %15s %10s" % ["Experiment", "Total", "Success", "Coverage %", "NHS Disc."]
      puts "-" * 80
      
      all_results.each do |result|
        puts "%-30s %10d %10d %14.2f%% %10d" % [
    ...
    def generate_summary(counters, total, successful, coverage_percentage)
      ...
      summary << "Too many matches: #{counters['too_many_matches_errors']}" if counters['too_many_matches_errors'] > 0
      summary << "Other errors: #{counters['other_errors']}" if counters['other_errors'] > 0
      summary << "NHS number discrepancies: #{counters['nhs_number_discrepancies']}" if counters['nhs_number_discrepancies'] > 0
