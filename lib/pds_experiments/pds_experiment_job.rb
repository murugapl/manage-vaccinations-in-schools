  ...
  class PDSExperimentJob < ApplicationJob
    def perform(patient, experiment_name)
      ...
      begin
        ...
        store_query_time(experiment_name, Time.zone.now - start_time)
        store_bad_request_ids(experiment_name, patient.id)
        return
    ...
    def store_family_name_discrepancy(experiment_name, patient, pds_patient)
      discrepancy_key =
        "pds_experiment:#{experiment_name}:family_name_discrepancy_ids"
    ...
    def store_bad_request_ids(experiment_name, patient_id)
      bad_request_key = "pds_experiment:#{experiment_name}:bad_request_ids"
      bad_requests = Rails.cache.read(bad_request_key) || []

      bad_requests << patient_id unless bad_requests.include?(patient_id)

      Rails.cache.write(bad_request_key, bad_requests, expires_in: 7.days)
    end
  end
end
